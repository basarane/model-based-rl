...
def plot_log(fname):
	...
	for step,ID in zip(range(args.idx[0], args.idx[2], args.idx[1]), range(0, 10000)):
		...
		s['current'] = np.expand_dims(np.linspace(-1, 1, 10000), axis=1)
		...
		for I in range(s['next'].shape[1]):
			...
			plt.scatter(s['current'][:,I],s['qvalue'][:], s=1, alpha=1) #, edgecolors='none'
			plt.scatter(s['current'][:,I],s['tderror'][:], s=1, alpha=1, c=colors[1]) #, edgecolors='none'
