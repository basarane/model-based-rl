...
basedir = 'td_offline_v_change_mountaincar'
...
def plot_log(fname):
	...
	for step,ID in zip(range(100, 50001, 100), range(0, 10000)):
		...
		v_model.model.load_weights('algo_convergence_mountain_car/td/train-0/weights_{}.h5'.format(step))
		...
		for I in range(s['next'].shape[1]):
			for J in range(s['next'].shape[1]):
				if I<J: #and J<K:# and I==0 and J==1: # and I==0 and J==2
					...
					plt.scatter(s['current'][:,I], s['current'][:,J], s=20, c=s['qvalue'][:], alpha=1, edgecolors='none', cmap=plt.get_cmap('viridis'))
