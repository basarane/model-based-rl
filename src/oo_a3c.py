...
class AgentThread(StoppableThread, RunnerListener):
	def __init__(self, threadId, sess, graph):
		...
		with self.graph.as_default():
			...
			#else:
			#	env = gym_env(args.game)
			env = gym_env(args.game + 'NoFrameskip-v4')
			...
			agentOps.FINAL_EXPLORATION_FRAME = int(1e6 / args.thread_count)
			...
			self.runner = Runner(env, agent, proproc, modelOps.AGENT_HISTORY_LENGTH)
			self.runner.listen(replay_buffer, proproc)
			self.runner.listen(agent, proproc)
