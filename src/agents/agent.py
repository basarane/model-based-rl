...
class DqnAgent(Agent, RunnerListener):
	def __init__(self, action_space, q_model, sampler, preproc, rewproc, ops):
	...
	def act(self, observation):
		...
		if (self.total_step_count > self.ops.REPLAY_START_SIZE) or self.ops.mode == "test":
			...
			if epsilon < random.random():
				if not self.preproc is None:
					observation = self.preproc.preprocess(observation)
	...
	def on_step(self, ob, action, next_ob, reward, done):
		...
		if self.total_step_count % self.ops.UPDATE_FREQUENCY == 0 and self.total_step_count>self.ops.REPLAY_START_SIZE:
			samples = self.sampler.get_sample(self.ops.MINIBATCH_SIZE)
			target = self.q_model.q_value([a['current_state'] for a in samples])
			next_value = self.q_model_eval.q_value([a['next_state'] for a in samples])
			...
			res = self.q_model.q_update([a['current_state'] for a in samples], target)
			#print (res, tot)
