...
class SequentialMemory(object):
	...
	def append(self, current_state, action, next_state, reward, done, current_state_orig = None, next_state_orig = None):
		#if len(self.buffer)>0:
		#	last = self.buffer[(self.buffer.current-1+len(self.buffer))%len(self.buffer)]
		#	if not last['done']:
		#		last['next_state'] = None
		#		last['next_state_orig'] = None
		self.buffer.append({
			...
			'reward': reward,
			'done': done,
			'current_state_orig': current_state_orig,
			'next_state_orig': next_state_orig
		})
